    ...
    for (int i = 0; i < reference_string.size(); i++)
    {
        ...
        if (string_is_inside_frame == false)
        {
            ...
            if (frame_has_empty_block == false)
            {
                ...
                for (int frame_index = 0; frame_index < frame.size(); frame_index++)
                {
                    for (int reference_string_index = i+1; reference_string_index < reference_string.size(); reference_string_index++)
                    {
                ...
                {
                    if (block_to_get_new_value[block_new_index] > maximum_value)
                    {
                        frame_index_for_new_value = block_new_index;
                        maximum_value = block_to_get_new_value[block_new_index];
                    }
                }
